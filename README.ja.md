...
- Codex は非対話実行、読み取り専用サンドボックス、承認なし、エフェメラルな最小セッションで実行します。
- 各CLIは失敗しても次の処理へ進みます。
- ネットワークに接続されていない場合は、CLIを起動せず `network` としてスキップします。
...
また、このツールは Claude Code / Codex CLI への軽量な疎通確認を行うため、実行時にMacがネットワークへ接続されている必要があります。オフラインの場合はAI CLIを起動せず、ログに `reason=network` を記録して終了します。
